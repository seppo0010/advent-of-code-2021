...
CREATE TABLE path_cave (cave VARCHAR(5), full_path TEXT);
...
SELECT to_cave, NEW.full_path || '-' || to_cave
FROM cave_conn 
WHERE
from_cave = NEW.cave AND
(NEW.cave = UPPER(NEW.cave) OR INSTR(NEW.full_path, '-' || NEW.cave || '-') = 0)
;
END;

INSERT INTO path_cave VALUES ('start', '-start');
