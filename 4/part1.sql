...
SELECT MIN(completed_in_number) FROM rows;
SELECT (
	(SELECT numbers.value FROM numbers WHERE numbers.id = (SELECT MIN(completed_in_number) FROM rows))
	* 
	(SELECT SUM(numbers.value) FROM numbers
	JOIN rows ON numbers.value IN (col0,col1,col2,col3,col4)
	WHERE board = (SELECT board FROM rows ORDER BY completed_in_number ASC LIMIT 1)
	AND numbers.id > (SELECT MIN(completed_in_number) FROM rows)
	)
);

SELECT MIN(completed_in_number) FROM cols;
SELECT (
	(SELECT numbers.value FROM numbers WHERE numbers.id = (SELECT MIN(completed_in_number) FROM cols))
	* 
	(SELECT SUM(numbers.value) FROM numbers
	JOIN cols ON numbers.value IN (row0,row1,row2,row3,row4)
	WHERE board = (SELECT board FROM cols ORDER BY completed_in_number ASC LIMIT 1)
	AND numbers.id > (SELECT MIN(completed_in_number) FROM cols)
	)
);
