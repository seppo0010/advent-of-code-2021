...
WITH RECURSIVE
  cnt(x) AS (
     ...
     UNION ALL
     SELECT x+1 FROM cnt
      LIMIT 1000000 -- max grid size :see_no_evil:
  )
...
UPDATE grid SET c = (
	SELECT COUNT(*) FROM lines WHERE (
		...
		(y = y1 AND y1 = y2 AND (x BETWEEN x1 AND x2 OR x BETWEEN x2 AND x1)) OR
		(x1 != x2 AND y1 != y2 AND (
			(x - x1) * (y2 - y) = (x2 - x) * (y - y1)
		) AND (
			(y BETWEEN y1 AND y2 OR y BETWEEN y2 AND y1) AND
			(x BETWEEN x1 AND x2 OR x BETWEEN x2 AND x1)
		))
	)
);
