...
CREATE TABLE octopus_coords (x INTEGER, y INTEGER, energy_level INTEGER, flash_count INTEGER, id INTEGER primary key autoincrement);
...
BEGIN
INSERT INTO octopus_coords (x, y, energy_level, flash_count) SELECT NEW.id, LENGTH(NEW.str) - 1, SUBSTR(NEW.str, LENGTH(NEW.str), 1), 0;
...
AFTER UPDATE ON iteration WHEN NEW.counter > 0
BEGIN
UPDATE octopus_coords SET energy_level = energy_level + 1;
UPDATE octopus_coords SET energy_level = 0, flash_count = flash_count + 1 WHERE energy_level > 9;
UPDATE iteration SET counter = counter - 1;
END;

INSERT INTO iteration VALUES(100); UPDATE iteration SET counter = counter;

SELECT SUM(flash_count) FROM octopus_coords;
